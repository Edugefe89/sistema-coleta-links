...
# --- FRAGMENTO DA TABELA (COM A CORREÇÃO DO SCROLL) ---
@st.fragment
def fragmento_tabela(id_p, lote, user, nome_p):
    ...
    # Sempre criamos uma CÓPIA (.copy()) para exibição.
    # Isso desvincula o objeto visual do objeto de memória e estabiliza o Streamlit.
    if foco:
        mask = (df_ref['link'] == "") | (df_ref['link'].isna())
        df_show = df_ref[mask].copy() # Já era copy/view
    else:
        df_show = df_ref.copy() # <--- AGORA É COPY TAMBÉM!
...
def tela_producao(user):
    ...
    if 'lote_ativo' not in st.session_state:
        ...
        if 'df_cache' not in st.session_state:
            ...
            # CRIA A COLUNA DE BUSCA AQUI (NO CARREGAMENTO)
